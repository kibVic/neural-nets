...
def prepare_data(input_size=150, batch_size=32):
    ...
    Adds error handling for missing directories or incorrect paths.
    """
    try:
...
def create_model(input_size=150, learning_rate=0.001, size_inner=100, droprate=0.5):
    ...
    Added error handling for invalid model configurations.
    """
    try:
...
def plot_history(history, epochs=10):
    ...
    Added error handling to ensure history is available and valid.
    """
    try:
...
def evaluate_model(model, test_data):
    ...
    Added error handling for model evaluation.
    """
    try:
...
def predict_image(model, image_path, input_size=299):
    ...
    Added error handling for image loading and prediction.
    """
    try:
...
def train_model(model, train_ds, val_ds, epochs=10, checkpoint_filepath='model_checkpoint.h5'):
    ...
    Added error handling for training issues.
    """
    try:
